...
#include <SFML/Graphics.hpp>
	...
	const int TRIPLE_BLOCK_HITS_REQUIRED = 3;     // Количество ударов для тройного блока
	const int UNBREAKABLE_BLOCKS_PER_SIDE = 2;   // Количество неубиваемых блоков с каждой стороны во втором ряду
	const int BONUS_SPAWN_CHANCE_PERCENT = 10;   // Вероятность выпадения бонуса при уничтожении блока (%)
	const int INITIAL_LIVES = 3;                  // Начальное количество жизней игрока
	const int MAX_PLAYER_NAME_LENGTH = 15;        // Максимальная длина имени игрока
	...
	// Размеры шрифтов UI
	const float UI_TITLE_FONT_SIZE = 48.0f;       // Размер шрифта заголовков попапов
	const float UI_SCORE_FONT_SIZE = 36.0f;       // Размер шрифта для отображения очков
	const float UI_RECORDS_FONT_SIZE = 20.0f;     // Размер шрифта для таблицы рекордов
	const float UI_MENU_FONT_SIZE = 30.0f;        // Размер шрифта для пунктов меню
	const float UI_GAME_INFO_FONT_SIZE = 24.0f;   // Размер шрифта для информации в игре (счет, жизни)
	const float UI_HINT_FONT_SIZE = 18.0f;        // Размер шрифта для подсказок
	const float UI_NAME_INPUT_FONT_SIZE = 28.0f;  // Размер шрифта для ввода имени
	
	// Позиции UI элементов
	const float UI_TITLE_Y = 50.0f;               // Позиция Y для заголовков попапов
	const float UI_SCORE_Y = 120.0f;              // Позиция Y для отображения очков
	const float UI_RECORDS_Y = 180.0f;            // Позиция Y для начала таблицы рекордов
	const float UI_RECORDS_START_X = 350.0f;      // Позиция X для таблицы рекордов
	const float UI_MENU_START_Y = 320.0f;         // Позиция Y для начала меню в попапах
	const float UI_MENU_DELTA_Y = 50.0f;          // Расстояние между пунктами меню
	const float UI_RECORDS_DELTA_Y = 22.0f;       // Расстояние между строками рекордов
	const float UI_GAME_INFO_X = 10.0f;           // Позиция X для информации в игре (счет, жизни)
	const float UI_GAME_INFO_SCORE_Y = 10.0f;     // Позиция Y для счета в игре
	const float UI_GAME_INFO_LIVES_Y = 40.0f;     // Позиция Y для жизней в игре
	const float UI_HINT_Y = 560.0f;               // Позиция Y для подсказок
	const float UI_HINT_START_X = 120.0f;         // Позиция X для начала подсказок
	
	// Цвета UI
	const sf::Color UI_HINT_COLOR = sf::Color(200, 200, 200);  // Цвет подсказок
